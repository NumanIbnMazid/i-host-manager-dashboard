<template>
  <div>
    <vs-table :data="theNewOrder.ordered_items">
      <template slot="thead">
      ...
      <template slot-scope="{ data }">
        ...
        <vs-tr v-if="payMethod == 2" class="bg-white font-bold">
          ...
          <vs-td class="text-right">
            ৳
            {{
              paidCash > theNewOrder.price.payable_amount
                ? (paidCash - theNewOrder.price.payable_amount).toFixed(2)
                : 0
            }}
          </vs-td>
        </vs-tr>
    ...
    <vs-button
      class="float-right"
      color="success"
      @click="createInvoice(theNewOrder)"
    >Print</vs-button
    >
      ...
      axios
        ...
        .then((res) => {
          if (res.data.status) {
            ...
            this.$emit("emitAfterNewOrderPayments", res);
          }
        })
        .catch((err) => {
